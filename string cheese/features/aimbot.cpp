...
namespace ap::features::ragebot
{
	...
	void run()
	{
		...
		auto pWeapon = /*(ap::sdk::c_base_weapon*)*/g::mango_local->get_active_weapon();
		...
		//
		//	float simtime = 0.f;
		//	vec3f minus_origin = vec3f(0.f, 0.f, 0.f);
		//	vec3f aim_position = run_aimscan(target, simtime, minus_origin);
		//
		//	if (aim_position == vec3f(0.f, 0.f, 0.f))
		//		continue;
		//
		//	float selection_value = 0.f;
		//	selection_value = fov;
		//
		//	if (best_distance >= selection_value && aim_position != vec3f(0.f, 0.f, 0.f)) {
		...
		//if (target_index != -1 && current_aim_position != vec3f(0.f, 0.f, 0.f) && pTarget) {

...
#define CLAMP(x, low, high)  (((x) > (high)) ? (high) : (((x) < (low)) ? (low) : (x)))
	...
	void clear() {
		...
		auto net_channel = *reinterpret_cast<ap::sdk::c_net_channel**>(reinterpret_cast<std::uintptr_t>(ap::interfaces::client_state) + 0x9C);
	...
	void legit(ap::sdk::c_user_cmd* mango_cmd) {
		...
		vec3f view_direction = ap::angle_vec3f(mango_cmd->viewangles + (mango_local->get_aim_punch() * 2.f));
		...
		for (int i = 1; i < 65; i++) {
			...
			if (!ap::interfaces::engine->get_player_info(i, &pInfo))
				continue;
			...
			if (!mango_entity->SetupBones(bone_data[i][mango_cmd->command_number % 12].bone_matrix, MAX_STUDIOBONES, BONE_USED_BY_HITBOX, ap::interfaces::globals->curtime))
				continue;
			...
			if (mango_entity->is_alive()) {
				...
				vec3f hitbox_position = ap::get_hitbox_position(mango_entity, 0);
		...
		if (best_target != -1) {
			...
			for (int t = 0; t < 12; ++t) {
				...
				/* temporary fov */
				float temp_fov_distance = point_to_line(entity_data[best_target][t].hitbox_position, mango_local->get_eye_position(), view_direction);

				if (temporary_float > temp_fov_distance && entity_data[best_target][t].simtime > mango_local->get_simulation_time() - 1) {
	...
	void update_backtrack() {
		...
		variables.latest_tick = ap::interfaces::globals->tickcount;
	}
	...
	void update_sequences() {
		...
		if (net_channel) {
			...
			if (net_channel->in_sequence_nr > variables.last_sequence) {
				...
				sequences.push_front(sequence_data(net_channel->in_reliable_state_count, net_channel->out_reliable_state_count, net_channel->in_sequence_nr, ap::interfaces::globals->realtime));
			}
	...
	void run(ap::sdk::c_user_cmd* mango_cmd) {
	...
	float latency()
	{
		const auto netch = ap::interfaces::engine->get_net_channel();
	...
	bool is_valid_backtrack_tick(float simtime)
	{
		...
		float delta_time = correct - (ap::interfaces::globals->curtime - simtime + lerp_time());

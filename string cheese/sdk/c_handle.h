...
public:
	...
	bool operator ==(const c_base_handle &other) const;
	bool operator ==(const c_handle_entity* pEnt) const;
	bool operator !=(const c_handle_entity* pEnt) const;
	bool operator <(const c_base_handle &other) const;
	bool operator <(const c_handle_entity* pEnt) const;

	// Assign a value to the handle.
	const c_base_handle& operator=(const c_handle_entity *pEntity);
	const c_base_handle& Set(const c_handle_entity *pEntity);
...
inline bool c_base_handle::operator ==(const c_handle_entity* pEnt) const
{
	return Get() == pEnt;
}
...
inline bool c_base_handle::operator <(const c_handle_entity *pEntity) const
{
	unsigned long otherIndex = (pEntity) ? pEntity->GetRefEHandle().m_Index : INVALID_EHANDLE_INDEX;
	return m_Index < otherIndex;
}

inline const c_base_handle& c_base_handle::operator=(const c_handle_entity *pEntity)
{
	return Set(pEntity);
}

inline const c_base_handle& c_base_handle::Set(const c_handle_entity *pEntity)
{
	if (pEntity)
